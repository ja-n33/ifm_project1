...
balance_plot <- ggplot(data = mx_long, aes(x = Year, y = value, fill = type)) +
    geom_bar(data = mx_long %>% filter(type %in% c(assetcols, liabilitycols)), 
    ...
    annotate("segment", x = 1992, xend = 2022, y = 0, yend = 0,
         color = "black", alpha = 0.5, linewidth = 0.75, linetype = "dashed") +
    ...
    scale_x_continuous(breaks = seq(from = 1992, to = 2022, by = 5), limits = c(1991, 2023), expand = c(0, 0)) + 
    geom_vline(xintercept = 1994, color = "#5F5E5A", alpha = 0.5,
             linewidth = 0.5, linetype = "dashed") + 
    geom_vline(xintercept = 2008, color = "#5F5E5A", alpha = 0.5,
             linewidth = 0.5, linetype = "dashed") +
    annotate("text", x = 1998, y = 500000,   label = "Assets",      
         hjust = 0, vjust = 0, size = 3, color = "#5F5E5A", family = "Georgia", fontface = "italic", face = "bold") +
    annotate("text", x = 1998, y = -500000,  label = "Liabilities",  
         hjust = 0, vjust = 1, size = 3, color = "#5F5E5A", family = "Georgia", fontface = "italic", face = "bold") +
    labs(title = "Mexican Financial Account Balance, 1992-2022", 
        caption = "Source: EWN Database\nNote: First dashed vertical line is Tequila Crisis, second is GFC", 
        y = "US$", 
        x = "Year") +
    theme_erasmus() +
    guides(fill = guide_legend(nrow = 2), 
          colour = guide_legend(nrow = 1)) +
    theme(plot.subtitle = element_text(size = rel(0.8), margin = margin(b = 2)),
